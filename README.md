...
관리자 화면에서는 Gemini 생성 기록과 검증 사유를 확인하고, 실패 작업 재시도, 생성 초안 편집·발행, 생성·검증 프롬프트 버전 관리, 사용자 관리자 권한 변경을 한곳에서 처리합니다.

프롬프트는 수정으로 덮어쓰지 않고 새 버전으로 저장합니다. 저장된 버전은 별도로 활성화해야 다음 생성 작업부터 적용되며, 이전 버전을 다시 활성화해 롤백할 수 있습니다. 생성 기록에는 실제 사용한 생성·검증 프롬프트 버전이 함께 표시됩니다.
