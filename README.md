...
/history        주간·월간 학습 기록
/wrong-answers  오답노트
/admin          운영 현황·퀴즈 생성·품질 관리·출처 검토·편집·권한·보안 운영
```
...
- 애플리케이션·DB·HTTPS 상태, 가동 시간, 배포 버전과 DB 크기 확인
- 향후 7일 퀴즈 재고, 최근 생성 품질·장애, 스케줄러와 DB 백업 상태 확인
- 실제 풀이 응답 수, 정답률과 선택지 분포를 이용한 문항 품질 확인
- 품질 상태·검색어 필터, 서버 페이지네이션과 문항별 상세 조회
- Gemini 호출 성공률·오류 코드·평균 응답 시간 확인
...
퀴즈 품질 화면은 Gemini를 다시 호출하지 않고 DB에 저장된 실제 풀이 결과만 집계합니다.

- `데이터 부족`: 응답이 5건 미만인 문항
- `검토 필요`: 정답률이 30% 미만 또는 90% 초과인 문항, 또는 오답이 3건 이상이고 특정 오답 선택률이 70% 이상인 문항
- `정상`: 충분한 응답이 쌓였고 위 검토 조건에 해당하지 않는 문항

품질 상태는 관리자의 검토 우선순위를 정하기 위한 지표입니다. 문항을 자동 삭제하거나 사용자 제공을 차단하지 않습니다.
...
- `dev`: 기능 개발 및 CI 대상
- `main`: 운영 배포 기준 브랜치
- 매일 오전 9시 또는 수동 실행으로 `dev`에서 `main`으로 승격 PR을 만들거나 기존 PR을 재사용합니다.
- 필수 PR 검사가 통과하면 자동 병합하고 `main` CI가 OCI 정적 배포를 수행합니다.
- 배포 마지막 단계에서 운영 HTML 스모크 테스트를 수행합니다.

```text
dev push -> dev CI -> 승격 PR -> 필수 검사 -> main 자동 병합 -> main CI -> OCI 배포 -> 운영 스모크 테스트
```
