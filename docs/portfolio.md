...
### 운영 정보를 여러 화면에서 찾아야 했던 문제

관리자 운영 현황에 퀴즈 재고, 실제 풀이 기반 문항 품질, DB 백업, 스케줄러와 Gemini 일일 사용량을 한 화면으로 모았습니다. 이상 항목은 숫자만 보여 주지 않고 해당 생성 기록이나 품질 검토 화면으로 바로 이동하게 해 운영 확인 시간을 줄였습니다.

### 일시 장애가 빈 데이터처럼 보였던 문제

오늘 퀴즈 조회가 실패하면 빈 상태와 재시도 동작을 명확히 보여 줍니다. Playwright에서는 첫 요청만 `503`을 반환한 뒤 사용자가 다시 확인했을 때 정상 선택 화면으로 복구되는 흐름을 데스크톱과 모바일에서 각각 검증합니다.

## 검증 근거

- `npm run build`: Next.js 프로덕션 빌드 성공 (2026-08-10 `dev`)
- `npm run test:e2e`: 29개 통과, 1개 의도적 제외
...
- 릴리스: [TRI:READ 프론트엔드 v1.0.0](https://github.com/TRI-READ/tri-read-fe/releases/tag/v1.0.0)
